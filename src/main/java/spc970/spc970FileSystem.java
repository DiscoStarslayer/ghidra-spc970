...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = spc970FileSystem.MyFileSystemFactory.class)
public class spc970FileSystem implements GFileSystem {
	 ...
	 */
	public spc970FileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + spc970FileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<spc970FileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public spc970FileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			spc970FileSystem fs = new spc970FileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
